...
function pam_render_location_meta_box( $post ) {
	...
	<p>
		...
		<label for="pam_auto_geocode">Automatically update latitude and longitude from address when saving (requires Google Maps API key)</label>
	</p>
...
function pam_save_location_meta_fields( $post_id ) {
	...
	if ( isset($_POST['pam_coordinates']) ) {
		error_log('Saving pam_coordinates: ' . $_POST['pam_coordinates']);
